// api/articulos/batch.js — POST multiple articles at once
...
module.exports = async (req, res) => {
  ...
  if (items.length > 500) {
    return res.status(400).json({ error: 'Max 500 items per batch' });
  }
      ...
      if (rows[0]) {
        ...
        if (Number(b.stock) > 0) {
          ...
          await sql`
            ...
            VALUES ('ENTRADA',${b.sku.toUpperCase()},${b.nombre},${Number(b.stock)},0,${Number(b.stock)},
                    ${b.created_by||'Sistema'},'Stock inicial - importación masiva',${fecha},${hora})`;
        }
      } else {
        skipped++; // duplicate
      }
  ...
  res.json({ ok: true, imported, skipped, errors: errors.slice(0,10), total: items.length });
};
