...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const rows = s
        ? await sql`SELECT * FROM articulos WHERE sku ILIKE ${'%'+s+'%'} OR nombre ILIKE ${'%'+s+'%'} OR categoria ILIKE ${'%'+s+'%'} OR ubicacion ILIKE ${'%'+s+'%'} ORDER BY nombre LIMIT 500`
        : await sql`SELECT * FROM articulos ORDER BY nombre LIMIT 500`;
