...
const INDUSTRY_CONFIGS = {
  GENERIC: {
    label: 'Generica',
    modules: Object.keys(MODULE_REGISTRY),
    inventoryTypes: ['GENERAL'],
    fields: [],
    dashboardWidgets: ['inventory_value', 'stock_alerts', 'purchase_orders'],
  },
  HARDWARE_STORE: {
    label: 'Ferreteria',
    modules: COMMON_MODULES.concat(['pedidos', 'generar-qr', 'escaner']),
    inventoryTypes: ['TORNILLERIA', 'HERRAMIENTAS', 'ELECTRICOS', 'PINTURAS', 'PVC', 'SEGURIDAD_INDUSTRIAL'],
    fields: ['marca', 'referencia', 'presentacion', 'calibre', 'medida', 'color', 'material', 'unidad_empaque', 'codigo_barras', 'compatibilidad'],
    dashboardWidgets: ['stock_alerts', 'reorder_points', 'category_value'],
  },
  METALWORKING: {
    label: 'Metalmecanica',
    modules: COMMON_MODULES.concat(['pedidos', 'logistica', 'traslados', 'generar-qr', 'escaner']),
    inventoryTypes: ['INSUMOS_REPUESTOS', 'LAMINAS_TUBERIAS', 'GASES_INDUSTRIALES'],
    fields: ['tipo_insumo', 'equipo_compatible', 'familia_material', 'material', 'norma', 'aleacion', 'espesor', 'calibre', 'diametro', 'largo', 'ancho', 'alto', 'peso_teorico', 'peso_real', 'estado_material', 'lote', 'certificado', 'tipo_gas', 'cilindro_id', 'psi_actual'],
    dashboardWidgets: ['critical_supplies', 'available_material', 'scraps', 'gas_recharge_alerts'],
  },
  CHEMICAL: {
    label: 'Quimica',
    modules: COMMON_MODULES.concat(['generar-qr', 'escaner']),
    inventoryTypes: ['MATERIAS_PRIMAS', 'REACTIVOS', 'PROCESO', 'TERMINADOS', 'RESIDUOS'],
    ...
    dashboardWidgets: ['expiration_alerts', 'blocked_batches', 'risk_summary'],
  },
  IMPORTS: {
    label: 'Importacion',
    modules: COMMON_MODULES.concat(['pedidos', 'integraciones']),
    inventoryTypes: ['IMPORTADOS', 'TRANSITO', 'NACIONALIZADOS'],
    fields: ['pais_origen', 'moneda', 'trm', 'incoterm', 'codigo_hs', 'peso_bruto', 'peso_neto', 'volumen', 'embarque', 'contenedor', 'costo_nacionalizado'],
    dashboardWidgets: ['shipments_in_transit', 'containers', 'landed_costs'],
  },
  AUTO_PARTS: {
    label: 'Repuestos',
    modules: COMMON_MODULES.concat(['pedidos', 'comercial', 'generar-qr', 'escaner']),
    inventoryTypes: ['REPUESTOS', 'CONSUMIBLES', 'ACCESORIOS'],
    fields: ['referencia_oem', 'referencias_alternas', 'marca_compatible', 'modelo_compatible', 'anio_inicial', 'anio_final', 'motor', 'cilindraje', 'posicion', 'lado'],
    dashboardWidgets: ['top_references', 'stockouts', 'compatibility_searches'],
  },
  TRANSPORT: {
    label: 'Transporte',
    modules: COMMON_MODULES.concat(['logistica', 'traslados']),
    inventoryTypes: ['BODEGA', 'TALLER', 'LLANTAS', 'BATERIAS', 'COMBUSTIBLE'],
    fields: ['placa', 'vehiculo', 'orden_trabajo', 'kilometraje', 'horas_motor', 'mecanico', 'centro_costo'],
    dashboardWidgets: ['open_work_orders', 'vehicle_costs', 'upcoming_maintenance'],
  },
  FOOD: {
    label: 'Alimentos',
    modules: COMMON_MODULES.concat(['pedidos', 'comercial']),
    inventoryTypes: ['MATERIAS_PRIMAS', 'INGREDIENTES', 'EMPAQUE', 'PROCESO', 'TERMINADOS', 'REFRIGERADOS', 'CONGELADOS'],
    fields: ['lote', 'fabricacion', 'vencimiento', 'temperatura', 'almacenamiento', 'conversion', 'alergenos', 'receta', 'rendimiento', 'merma'],
    dashboardWidgets: ['expiration_alerts', 'batch_traceability', 'waste_costs'],
  },
};
...
function resolveCompanyConfig(company) {
  ...
  return {
    ...
    modules,
    inventoryTypes: industry.inventoryTypes,
    fields: industry.fields,
    dashboardWidgets: industry.dashboardWidgets,
  };
}
